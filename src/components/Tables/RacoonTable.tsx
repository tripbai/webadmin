...
export default function RacoonTable({headings, rows, isLoading, loadingSkeleton, emptyMessage, searchSubmit, hasNextPage, hasPreviousPage}: Props) {
    ...
    const renderRows = () => {
        if (isLoading) {
            return Array.from({ length: loadingSkeleton.rows }).map((_, rowIndex) => (
                <tr key={rowIndex} className="">
                    {Array.from({ length: loadingSkeleton.columns }).map((_, cellIndex) => (
        ...
        if (rows.length === 0) {
            return (
                <tr>
                    <td colSpan={headings.length + (hasButtons ? 1 : 0)} className="text-center py-3">
                        {emptyMessage}
                    </td>
                </tr>
            );
        ...
        return rows.map((row, index) => (
            <tr key={index}>
                {row.cells.map((cell, cellIndex) => (
                    <td key={cellIndex} className="px-6 py-3 whitespace-nowrap dark:text-gray-200">
                        {cell}
                    </td>
                ))}
        ...
        <div className="mt-12 shadow-sm border dark:border-gray-700 rounded-lg overflow-x-auto">
            <section className="w-full flex">
                <div className="py-3 px-4 w-1/2">
                    <form onSubmit={e=> onSearchSubmit(e)} className='flex-1 items-center justify-start lg:flex lg:pb-0 w-full'>
                ...
                <div className="py-3 px-4 w-1/2 flex flex-row-reverse">
                    <div>
                        <NavigationArrows 
                            leftArrow={{ isActive: hasPreviousPage && !isLoading }} // Placeholder for actual logic
                            rightArrow={{ isActive: hasNextPage && !isLoading }} // Placeholder for actual logic
                        />
                    </div>
                </div>
            </section>
