...
export default function NavigationArrows({ leftArrow, rightArrow }: Props) {
  ...
  const activeIconClass =
    "text-gray-800 dark:text-gray-200 w-6 h-6 cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400 duration-150";
  const leftArrowIcon = (
    <svg
      className={leftArrow.isActive ? activeIconClass : inactiveIconClass}
  ...
  const rightArrowIcon = (
    <svg
      className={rightArrow.isActive ? activeIconClass : inactiveIconClass}
      xmlns="http://www.w3.org/2000/svg"
      fill={"currentColor"}
      viewBox="0 0 24 24"
    >
