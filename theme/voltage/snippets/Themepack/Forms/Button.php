...
// If Icon right is set
if (isset($snippet["icon:right"])) {
    $snippet["icon:right"]["width"] ??= "concrete-width-extra-small:11";
    $snippet["icon:right"]["stroke:color"] ??= "svg-stroke:white";
    $snippet["icon:right"]["fill:color"] ??= "svg-fill:none";
    $snippet["icon:right"]["classlist"] ??= [];
    $__icon_classlist = array_merge(
        $snippet["icon:right"]["classlist"],
        [
            $snippet["icon:right"]["width"],
            $snippet["icon:right"]["stroke:color"],
            $snippet["icon:right"]["fill:color"],
            "margin-left:4",
        ]
    );
    ob_start();
    snippet("Themepack/Icons/SVG", [
        ...
        "use:gradient" => $snippet["icon:right"]["use:gradient"] ?? false,
    ]);
    $html["@icon:right"] = ob_get_contents();
    ob_end_clean();
}
